...
- (NSString *)generateProperty:(id)a
{
...
//
	...
	for (NSValue *inputRangeValue in ranges) {
        ...
        DDLogVerbose(@"outGenerateType %@",[self outGenerateType:substring]);
	}
...
+ (NSString *)outGenerateType:(NSString *)Line
{
    NSRange range = [Line rangeOfString:@"*"];//判断字符串是否包含
    if (range.length >0){
        [self outPointerGenerate:Line];
    }else{
        [self outNoPointerGenerate:Line];
    }
    return nil;
}

//指针类型的输出
+ (NSString *)outPointerGenerate:(NSString *)Line
{
	NSString *tempStr = [Line stringByReplacingOccurrencesOfString:@";" withString:@""];
	NSString  *outType = [PROPERTY stringByAppendingFormat:@" (%@,",NONATOMIC];
	NSArray  *typeArray = [NSArray arrayWithArray:[tempStr componentsSeparatedByString:@"*"]];
	
	NSString    *firstString = [typeArray firstObject];
	NSString    *lastString = [typeArray lastObject];
	
	outType = [outType stringByAppendingFormat:@"%@)",[self FirstString:firstString]];
	
	firstString = [firstString stringByReplacingOccurrencesOfString:@"__weak" withString:@""];
	firstString = [firstString stringByReplacingOccurrencesOfString:@"__strong" withString:@""];
	
	outType = [outType stringByAppendingFormat:@"%@",firstString];
	outType = [outType stringByAppendingFormat:@"%@;",lastString];
	
	return outType;
}

+ (NSString *)FirstString:(NSString *)aStr
{
    NSRange range = [aStr rangeOfString:@"__weak"];//判断字符串是否包含
    if (range.length >0){
        return WEAK;
    }else{
        return STRONG;
...
//非指针类型的输出
+ (NSString *)outNoPointerGenerate:(NSString *)Line
{
    NSString  *outType = [PROPERTY stringByAppendingFormat:@" (%@,%@)",NONATOMIC,ASSIGN];
    NSMutableArray  *tempLineArray = [NSMutableArray arrayWithArray:[Line componentsSeparatedByString:@" "]];
    for (int j = 0; j < [tempLineArray count]; j++) {
        if ([[tempLineArray objectAtIndex:j] isEqualToString:@""]) {
            [tempLineArray removeObjectAtIndex:j];
        }
    }
    NSString    *firstString = [tempLineArray firstObject];
    NSString    *lastString = [tempLineArray lastObject];
